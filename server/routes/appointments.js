...
router.get('/', auth, getAppointments) // need to add auth, to not show to all users
router.post('/', createAppointment)//no login needed
router.patch('/:id', auth, updateAppointment)// any logged in can update post
router.delete('/:id', deleteAppointment)//delete button can remain for all loged in users
